...
# Serve frontend (MUST be last)
# Local run: repo root has /frontend
# Docker run: Dockerfile copies /frontend into the image
repo_frontend_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "frontend"))
docker_frontend_dir = "/frontend"

if os.path.isdir(repo_frontend_dir):
    ...
    # Don’t crash the app if frontend isn’t present; API can still run
    print(f"[WARN] Frontend directory not found. tried: {repo_frontend_dir} and {docker_frontend_dir}")
